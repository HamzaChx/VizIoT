...
// let xBuffer = []; // Buffer for X-axis values
// let yBuffer = []; // Buffer for Y-axis values
// let currentIndex = 0; // Tracks the current index for animation
...
// /**
//  * Processes incoming graph data and appends it to the buffers.
//  * @param {Array} newGraphData - New data points to plot.
//  */
// function updateBuffers(newGraphData) {
//     const x = newGraphData.map((point) => point.x);
//     const y = newGraphData.map((point) => point.y);
...
// /**
//  * Starts the incremental drawing of the graph.
//  */
...
//         if (currentIndex >= xBuffer.length) {
//             // Wait for new data
//             requestAnimationFrame(drawIncrementally);
//             return;
//         }

//         // Determine the current plotting range
//         const xMin = Math.min(...xBuffer);
//         const xMax = Math.max(...xBuffer);
...
//         // // Plot event markers for the current range
//         // events.forEach((event) => {
//         //     const eventTime = new Date(event.timestamp).getTime() / 1000; // Convert to seconds
//         //     if (eventTime >= xMin && eventTime <= xBuffer[currentIndex]) {
//         //         gr.setmarkercolorind(2);
//         //         gr.polymarker(1, [eventTime], [yBuffer[0]]); // Adjust event Y coordinate as needed
//         //     }
//         // });
