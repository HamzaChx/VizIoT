...
async function storeSensorEvents(processEvents) {
  ...
  try {
    ...
    for (const event of processEvents) {
      ...
      await db.run(
        'INSERT OR IGNORE INTO ProcessEvents (name, ranking, description) VALUES (?, ?, ?)',
        [name, ranking, meta?.description || '']
      );

      // Retrieve the event ID
      const processEvent = await db.get('SELECT event_id FROM ProcessEvents WHERE name = ?', [name]);
      if (!processEvent) {
